...
public class ValidationConfiguration {
    ...
    @Bean
    public ValidationArgumentResolver validationArgumentResolver(ResourceBundleMessageSource messageSource) {
        ...
        return new ValidationArgumentResolver(
                validationEngine(validatorRegistry(), messageResolver(messageSource)),
                mode,
                enableI18n,
                messageConverters
        );
    ...
    @Bean
    public SmartInitializingSingleton argumentResolverInitializer(RequestMappingHandlerAdapter adapter, ResourceBundleMessageSource messageSource) {
        return () -> {
            List<HandlerMethodArgumentResolver> resolvers = new ArrayList<>();
            // 将自定义解析器添加到最前面
            resolvers.add(validationArgumentResolver(messageSource));
            // 再添加原有的解析器
